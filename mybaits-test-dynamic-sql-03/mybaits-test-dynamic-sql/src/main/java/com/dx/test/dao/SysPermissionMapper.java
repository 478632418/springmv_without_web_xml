...
public interface SysPermissionMapper {
	...
	@Insert(value = { //
			...
			"(`permission_name`,`permission_value`,`permission_url`,`description`,`create_time`,`create_user`,`create_user_id`,`modify_time`,`modify_user`,`modify_user_id`,`version`)",
			"VALUES", //
			"<foreach collection=\"sysPermissionList\" item=\"item\" index=\"index\" separator=\",\">", //
			"     (#{item.permissionName},#{item.permissionValue},#{item.permissionUrl},#{item.description},now(),#{item.createUser},#{item.createUserId},now(),#{item.updateUser},#{item.updateUserId},0)", //
			"</foreach>", //
	...
	@Update(value = { //
			...
			"  </if> ", //
			" <if test='item.permissionUrl !=null and item.permissionUrl != \"\"'>", //
			"       ,`permission_url` = #{item.permissionUrl} ", //
			"  </if> ", //
